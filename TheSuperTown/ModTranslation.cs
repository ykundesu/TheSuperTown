...
namespace TheSuperTown
{
    public class ModTranslation
    {
        ...
        public static void Load()
        {
            ...
            Stream stream = assembly.GetManifestResourceStream("TheSuperTown.Resources.translatedate.json");
            ...
            for (int i = 0; i < parsed.Count; i++)
            {
                ...
                if (token.HasValues)
                {
                    ...
                    for (int j = 0; j < (int)SupportedLangs.Irish + 1; j++)
                    {
                        ...
                        {
                            //TheSuperTownPlugin.Instance.Log.LogInfo($"key: {stringName} {key} {text}");
                            strings[j] = text;
                        }
                    }
        ...
        public static Sprite getImage(string key, float pixelsPerUnit)
        {
            key = key.Replace("/", ".");
            key = key.Replace("\\", ".");
            key = "TheSuperTown.Resources." + key;
