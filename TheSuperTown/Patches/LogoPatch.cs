...
namespace TheSuperTown.Patches
{
    [HarmonyPatch]
    public static class CredentialsPatch
    {

        public static string baseCredentials = $@"<size=130%><color=#ffa500>Super</color><color=#ff0000>New</color><color=#00ff00>Roles</color></size> v{TheSuperTownPlugin.Version.ToString()}";
        ...
        private static class VersionShowerPatch
        {
            ...
            static void Postfix(VersionShower __instance)
            {
                ...
                version.SetText(string.Format(ModTranslation.getString("creditsVersion"), TheSuperTownPlugin.Version.ToString()));
        ...
        private static class LogoPatch
        {
            static void Postfix(PingTracker __instance)
            {
                ...
                var tstLogo = new GameObject("bannerLogo_TST");
                tstLogo.transform.position = Vector3.up;
                var renderer = tstLogo.AddComponent<SpriteRenderer>();
                renderer.sprite = ModTranslation.getImage("Banner.png", 150f);

                if (File.Exists(Assembly.GetExecutingAssembly().Location.Replace("TheSuperTown.dll", "Submerged.dll"))) return;
                TheSuperTownPlugin.Logger.LogInfo("通過ぁぁぁ！:" + Assembly.GetExecutingAssembly().Location.Replace("TheSuperTown.dll", "Submerged.dll"));
                //サブマージド追加ボタン
                ...
                __instance.StartCoroutine(Effects.Lerp(0.1f, new System.Action<float>((p) => {
                    text.SetText(ModTranslation.getString("SubmergedDownload"));
                })));
                ...
                void onClick()
                {
                    TheSuperTownPlugin.Logger.LogInfo("ダウンロード！");
                    showPopup(ModTranslation.getString("SubmergedDownloaded"));
                    DownloadSubmarged(button);
                }
            }

            private static Task DownloadTask = null;
            public static async Task<bool> DownloadSubmarged(GameObject button)
            {
                try
                {

                    HttpClient httpa = new HttpClient();
                    httpa.DefaultRequestHeaders.Add("User-Agent", "TheSuperTown Downloader");
                    ...
                    http.DefaultRequestHeaders.Add("User-Agent", "TheSuperTown Downloader");
                    ...
                    string code = Assembly.GetExecutingAssembly().Location.Replace("TheSuperTown.dll", "Submerged.dll");
                    ...
                    showPopup(ModTranslation.getString("SubmergedDownloadSuc"));
                    button.SetActive(false);
                    return true;
                }
                catch (System.Exception ex)
                {
                    TheSuperTownPlugin.Instance.Log.LogError(ex.ToString());
